// AUTO-GENERATED RADAR MASTER DATA FROM USER EXCEL IMAGE (TCT & SCVN M1 -> M7)
...
export function getRadarScores(periodType: string, monthStr: number | string, quarterStr: number | string, yearStr: number | string) {
  const month = Number(monthStr) || 6;
  const quarter = Number(quarterStr) || 2;
  const year = Number(yearStr) || 2026;

  let currKey = "m6_26";
  let prevKey = "m5_26";
  let labelCurr = "Tháng 6/2026";
  let labelPrev = "Tháng 5/2026";
  ...
  if (periodType === "weekly" || periodType === "monthly") {
    ...
    labelPrev = quarter > 1 ? `Quý ${quarter - 1}/${year}` : `Quý 4/${year - 1}`;
  } else {
    currKey = `year_2026`;
    prevKey = `year_2025`;
    labelCurr = `Năm ${year}`;
  ...
  for (const k of keys) {
    const scvnRec = RADAR_MASTER_DATA["SCVN"]?.[k] || {};
    const tctRec = RADAR_MASTER_DATA["TCT"]?.[k] || {};
